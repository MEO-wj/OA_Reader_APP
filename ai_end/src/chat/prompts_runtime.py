...
【输出约束】
- 当返回多条帖子、通知或检索结果时，优先按分层列表输出，不要默认使用 Markdown 表格
- 如果用户明确要求“用表格展示”，且信息天然适合按列对比，应优先使用正确的 Markdown 表格
- 一旦决定使用表格，就保持标准 Markdown 表格格式，不要混用“帖子卡片 + 表格字段”两套样式
- 如果用户是在追问某一条帖子的具体内容、条件或流程，优先用“小标题 + 段落/分点”讲解，不要写成“项目 | 内容”两列表格
- 每条结果单独成段，使用“标题 + 关键信息 + 你需要关注”结构，保证移动端易读
- 每条结果至少写出“适用对象 / 关键条件 / 关键时间或金额”三类信息中的两类，不要只给一句很短的摘要
- 如果是通知解读，优先写成 2-3 句完整说明，而不是几个并列短词
- 只有在内容主要是长段解释、无法按列归纳时，才不要强行使用 Markdown 表格
...
{{
    "confirmed": {{
        "identity": ["用户明确告知的身份信息"],
        "interests": ["用户明确表达的偏好方向"],
        "constraints": ["用户明确提出的硬性约束"]
    }},
    "hypothesized": {{
...
# Step 2: 将新画像与旧画像合并
PORTRAIT_MERGE_PROMPT = """你是画像合并助手。请将旧画像 JSON 与新画像 JSON 合并为一份完整画像。

## 合并规则
1. 旧画像中未冲突的字段直接保留；冲突时新信息优先
2. 去重：新旧列表中重复的条目只保留一份
3. hypothesized 不升入 confirmed：即使新画像中将某项标为 confirmed，若旧画像中该项为 hypothesized，合并后仍归入 hypothesized，除非用户在对话中明确确认
4. 空字段保留旧值：若新画像某字段为空，沿用旧画像对应字段的值
