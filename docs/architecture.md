...
| **ai_end** | Python + Flask + LangGraph + LangChain | 4421 | AI 服务、RAG 问答 |
...
flowchart TB
    ...
    subgraph 服务层["⚙️ 服务层"]
        AIEnd["AI_end (Flask:4421)<br/>RAG Agent / 向量搜索"]
...
sequenceDiagram
    ...
    participant AI as AI_end (Flask)
...
flowchart TD
    ...
    E --> F[加载历史对话<br/>Redis 24h TTL]
    F --> G[LangGraph Agent 执行]
    G --> H{LLM 判断<br/>是否需要检索?}
    H -->|需要| I[调用 vector_search tool]
    H -->|不需要| N[直接生成回答]

    I --> J[生成问题向量<br/>POST /embed]
    J --> K[pgvector 相似度搜索]
    K --> L[时效性加权排序]
    L --> M[返回相关文章]
    M --> N
...
def vector_search_tool(query: str, top_k: int = 3, detail_level: str = "brief") -> str:
    """OA 向量检索工具：返回相关文章内容与摘要"""
    embedding = generate_embedding(query)
    articles = search_similar_articles(embedding, top_k)
    return json.dumps({"documents": articles, "related_articles": ...})

def build_agent():
    graph = StateGraph(AgentState)
    graph.add_node("agent", agent_node)      # LLM 决策节点
    graph.add_node("tools", ToolNode([vector_search_tool]))  # 工具节点
...
| AI 短期记忆 | Redis | `ai:mem:{user_id}` | 24h | 服务端存储，最近 5 轮对话 |
