...
运行方式：
    ...
    # 或手动指定 DATABASE_URL:
    DATABASE_URL="postgresql://..." uv run pytest tests/test_transaction.py -v
"""
...
# 自动从 .env 文件加载 DATABASE_URL（如果未设置环境变量）
def _load_env():
...
class TestPipelineTransactionFlow:
    ...
    def test_single_article_rollback_isolation(self):
        ...
        link_a = f"https://example.com/article_a_{today}"
        link_b = f"https://example.com/article_b_{today}"
        link_c = f"https://example.com/article_c_{today}"
        ...
        try:
            ...
            # 验证结果
            existing = repo.existing_links(conn, today)
...
class TestPipelineRunRegression:
    """Pipeline.run 回归测试（针对代码审查提出的事务问题）"""

    def _build_pipeline_with_mocks(self, monkeypatch):
        from crawler.pipeline import Crawler
        import crawler.db as db_module
        ...
        repo = MagicMock()
        repo.ensure_schema = MagicMock()
        repo.existing_links = MagicMock(return_value=set())

        monkeypatch.setattr(db_module, "get_connection", lambda: conn)

        crawler = Crawler(target_date="2026-03-19", enable_delay=False)
        crawler.repo = repo
        crawler._within_hours = lambda: True
        crawler._fill_summaries = lambda items: [item.update({"摘要": "测试摘要"}) for item in items]

        return crawler, conn, repo

    def test_run_rolls_back_when_article_id_missing_and_continues(self, monkeypatch):
        """插入成功但取不到文章ID时，应回滚当前事务且继续处理后续文章"""
        from crawler import pipeline as pipeline_module

        crawler, conn, repo = self._build_pipeline_with_mocks(monkeypatch)

        candidates = [
            SimpleNamespace(title="A", unit="U", link="https://example.com/a", published_on="2026-03-19"),
            SimpleNamespace(title="B", unit="U", link="https://example.com/b", published_on="2026-03-19"),
        ]
        monkeypatch.setattr(pipeline_module, "fetch_list", lambda _: candidates)
        monkeypatch.setattr(
            pipeline_module,
            "fetch_detail",
    ...
    def test_run_embedding_failure_does_not_abort_batch(self, monkeypatch):
        ...
        candidates = [
            SimpleNamespace(title="A", unit="U", link="https://example.com/a1", published_on="2026-03-19"),
            SimpleNamespace(title="B", unit="U", link="https://example.com/b1", published_on="2026-03-19"),
        ]
        monkeypatch.setattr(pipeline_module, "fetch_list", lambda _: candidates)
        monkeypatch.setattr(
            pipeline_module,
