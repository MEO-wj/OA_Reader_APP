...
func TestAsk_InjectsUserIDIntoForwardedBody(t *testing.T) {
	...
	if gotPath != "/ask" {
		t.Fatalf("expected /ask to proxy to /ask, got %s", gotPath)
	}
	...
	if gotBody["question"] != "hi" {
		t.Fatalf("expected question=hi to be preserved, got %v", gotBody["question"])
	}
}

func TestAsk_PreservesCompatFields(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	req := httptest.NewRequest(http.MethodPost, "/api/ai/ask",
		io.NopCloser(bytes.NewBufferString(`{"question":"hi","top_k":10,"conversation_id":"conv-1"}`)))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)

	if gotBody["top_k"] != float64(10) {
		t.Fatalf("expected top_k to be preserved, got %v", gotBody["top_k"])
	}
